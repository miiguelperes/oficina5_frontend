  ...
  <div class="text-center">
    <v-dialog v-model="dialog" width="500">
      <template v-slot:activator="{ on }">
        <v-btn  v-on="on">Editar</v-btn>
      </template>

      <v-card>
        <v-card-title class="headline grey lighten-2" primary-title>Editar Post</v-card-title>
...
export default {
  ...
  data: () => ({
    ...
    created() {
        ...
        self.body = self.post.body;
    },
  methods: {
    sendPost(){
      var self = this;
      this.axios.put(self.url+'posts/'+self.post.id, {userId: 1, title: self.title, body: self.body}).then((response) => {
